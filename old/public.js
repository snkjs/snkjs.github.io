...
var tmp_arr=Domain.match(/[\w][\w-]*\.(?:com\.cn|net\.cn|org\.cn|com|cn|co|net|org|gov|cc|biz|info|hn|xyz|hk|us|mobi|wang|me|so|top|win|vip)(\/|$)/);
Domain=tmp_arr[0];

var tmp_url = location.href;
if(tmp_url.indexOf("baike_add")==-1 && tmp_url.indexOf("bbs_add_book")==-1 && tmp_url.indexOf("bbs_thread")==-1 && tmp_url.indexOf("baike_edit")==-1 && tmp_url.indexOf("bbs_edit_book")==-1 && tmp_url.indexOf("/baike_")==-1 && tmp_url.indexOf("/thread_")==-1 && tmp_url.indexOf("/dom/sc_")==-1 && tmp_url.indexOf("/forum_")==-1 && tmp_url.indexOf("/dom/form_iframe.php")==-1 && tmp_url.indexOf("/zd_")==-1 && tmp_url.indexOf("/dom/zhidao")==-1){
	...
	$.post(
        ...
        function (data) {
			if (!data || data == '') {
				...
				$("#shop_price").html('<s class="noPrice">暂无价格</s>');
				$("#Real_price").val(0);
                $("#pro_stock").html(0);
            ...
            if ($("#big_pic_guige").length > 0 && data.imgPath) {
            	...
            	$("#big_pic_guige").attr('bsrc', data.imgBigPath);
            }
            ...
            if (price <= 0) {
                show_parice = '<s class="noPrice">暂无价格</s>';
                $("#jifen").hide();
                $("#shopUnit").hide();
            } else {
                show_parice = '￥'+ price;
